...
ggplot(fitData, aes(x=category, fill=is.na(flag_algorithm))) + geom_bar()  + 
  scale_fill_manual(values=c("TRUE" = "#33CC00", "FALSE" = "#CC0000")) +  
...
gf <- gf[ , Category := fitData[goodnessOfFitExists,]$category]
gf <- as.data.frame(gf)
...
#  g <- g + geom_dotplot(binwidth =   diff(range(data["value"]))/30,
  ...
  g <- g + scale_fill_gradient(low = ifelse(variable!="rSquared","green","red"), high = ifelse(variable!="rSquared","red","green")) + scale_y_continuous(name = "", breaks = NULL)
  g
}) 
grid.newpage()
